  ...
  <div>
    <el-row>
      ...
      <el-col :span="2">
        ...
        <el-dialog title="添加员工" :visible.sync="dialogFormVisible" width="30%">
          <el-form :model="form" ref="form1" :rules="rules">
            ...
            <el-form-item label="教育水平：" :label-width="formLabelWidth" prop="education">
              <el-select v-model="form.education" placeholder="请选择" style="width: 220px">
                <el-option
            ...
            <el-form-item label="工资：" :label-width="formLabelWidth" prop="wages">
              <el-input v-model="form.wages" autocomplete="off" style="width: 220px"></el-input>
            </el-form-item>
            <el-form-item label="入职日期：" :label-width="formLabelWidth" prop="entry_time">
        ...
        <!-- 添加员工人脸信息 -->
        <el-dialog title="添加员工-人脸信息" :visible.sync="dialogFormVisible1" width="30%">
          <div>
    ...
    >
      ...
      <el-table-column label="备注" width="487%">
        <template slot-scope="scope">
          <!--          <i class="el-icon-time"></i>-->
      ...
      <el-table-column label="操作" width="190%">
        <template slot-scope="scope">
          ...
          <el-popconfirm title="确定离职吗？" @confirm="handleDelete(scope.$index, scope.row)">
            <el-button
              size="mini"
              type="danger"
              slot="reference"
...
export default {
  data() {
    ...
    return {
      base_face_img_url: "http://127.0.0.1:8000/api/staff_img/",
  ...
  methods: {
    ...
    handleDelete(index, row) {
      console.log(index, row);
      this.$api
        .del_staff_info({}, { type: "json", urlParam: row.id })
        .then(response => {
          if (response.state === 1) {
    ...
    nextStep() {
      ...
      this.face_feature_img_url =
        this.base_face_feature_img_url + "staff_" + id;
    },
      ...
      let param = {
        id:
          this.tableData.length == 0
            ? 1
            : this.tableData[this.tableData.length - 1].id + 1,
        gender: this.form.gender == "mail" ? 1 : 0,
        entry_time:
        ...
        // resignation_time: "2020-01-01",
        wages: parseInt(this.form.wages),
